...
async function getMetadataFromCargo() {
  ...
  try {
    ...
    if (await fs.pathExists(metadataPath)) {
      ...
      if (!metadata.name) {
        throw new Error("metadata.txt must contain a 'name' field");
      }
      return metadata;
    }
    ...
    if (!cargoData.package || !cargoData.package.name) {
      throw new Error("Cargo.toml must contain a package name");
    }

    return {
      name: cargoData.package.name.replace(/"/g, ""),
      description: (cargoData.package.description || "").replace(/"/g, ""),
      version: (cargoData.package.version || "0.1.0").replace(/"/g, ""),

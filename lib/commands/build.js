...
async function buildWasm(projectPath, spinner) {
  ...
  try {
    ...
    try {
      ...
      // Rename the WASM file to ownable.wasm
      const originalWasmPath = path.join(
        projectPath,
        "target",
      ...
      const renamedWasmPath = path.join(
        projectPath,
        "target",
        "wasm32-unknown-unknown",
        "release",
      ...
      await fs.move(originalWasmPath, renamedWasmPath, { overwrite: true });
    } catch (error) {

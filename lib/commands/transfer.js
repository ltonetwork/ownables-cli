...
async function createEventAndChainJson(
  ...
  // Only add creators if they are provided and not empty
  if (creators) {
    const trimmedCreator = String(creators).trim();
    if (trimmedCreator) {
      // Take only the first creator name
      const firstCreator = trimmedCreator.split(",")[0].trim();
      if (firstCreator) {
        msg.creators = [firstCreator];
      }
    }
  }
...
async function transfer() {
  ...
  try {
    ...
    const { recipient, creators, relay, network, transferCount } =
      await inquirer.prompt([
        ...
        },
        {
          type: "input",
          name: "creators",
          message: "Enter creators (comma-separated names):",
          default: null,
        },
        {
          type: "list",

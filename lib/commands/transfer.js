...
/**
 * Calculate CID of a ZIP file
 */
async function calculateZipCID(zipBuffer, spinner) {
  try {
    ...
    // Debug: List all files in the ZIP
    spinner.info("Files in ZIP:");
    Object.keys(zip.files).forEach((filename) => {
      spinner.info(chalk.cyan(`  - ${filename}`));
    });

    // Extract files from ZIP, excluding chain.json and hidden files
    for (const [filename, file] of Object.entries(zip.files)) {
      if (
        ...
        spinner.info(chalk.green(`Including file: ${filename}`));
      } else {
        spinner.info(chalk.yellow(`Excluding file: ${filename}`));
      }
    }

    // Debug: Show final files being used for CID
    spinner.info("Files used for CID calculation:");
    files.forEach((file) => {
      spinner.info(chalk.cyan(`  - ${file.path}`));
    });

    // Calculate CID using ipfs-unixfs-importer
    let lastCid;
    for await (const { cid } of importer(files, { onlyHash: true })) {
      lastCid = cid;
    }
    ...
    spinner.succeed(`CID calculated: ${lastCid}`);
...
async function transfer() {
  ...
  try {
    ...
    // Calculate CID of the ZIP file
    const packageCID = await calculateZipCID(zipBuffer, spinner);
    ...
    for (let i = 0; i < transferCount; i++) {
      ...
      // Pass the calculated CID to the function
      const { chainJson, anchorTx } = await createEventAndChainJson(
        ...
        recipient,
        spinner,
        packageCID, // Pass the calculated CID
        existingChain
      );

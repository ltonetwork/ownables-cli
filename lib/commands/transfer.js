...
async function calculatePackageCID(zip) {
  // Create a new zip without chain.json
  const newZip = new JSZip();

  // Copy all files except chain.json
  for (const [filename, file] of Object.entries(zip.files)) {
    if (filename !== "chain.json") {
      newZip.file(filename, await file.async("nodebuffer"));
    }
  }
  ...
  const result = await ipfs.add(zipBuffer);
...
async function createEventAndChainJson(
  ...
  // Calculate package CID
  spinner.text = "Calculating package CID...";
  const packageCID = await calculatePackageCID(zip);

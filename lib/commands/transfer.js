...
async function calculatePackageCID(zipPath) {
  // Create a temporary directory for the filtered package
  const tempDir = path.join(os.tmpdir(), "ownable-package-" + Date.now());
  ...
  try {
    ...
    // Add all files except hidden files and chain.json
    ...
    const tempZipPath = path.join(tempDir, "package.zip");
    await fs.writeFile(tempZipPath, newZipContent);

    // Use ipfs command to add the file and get its CID
    const { stdout } = await execAsync(`ipfs add -Q ${tempZipPath}`);
    return stdout.trim();
  } finally {
